...
def test_create_signup_and_token():
    result = create_business_signup(
        ...
        specialization="CNC onderhoud en storingsdienst",
        tier="starter",
    )
...
def test_create_signup_with_email():
    result = create_business_signup(
        ...
        specialization="warmtepomp installateur",
...
def test_demo_request():
    req_id = record_demo_request(name="Jane", email="jane@example.com", business_name="InstallPro")
    assert req_id > 0


def test_setup_url_format():
    url = setup_url("industrial-abc", "tok123")
    assert "tenant=industrial-abc" in url
